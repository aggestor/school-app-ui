    ...
    <div class="w-full h-full grid place-items-center">
        <div class="w-[95%] rounded shadow flex flex-col pt-2 pb-4 px-4 h-auto py-6 bg-white">
            <div class="w-full sticky top-0 bg-white flex items-center justify-between h-14 mb-3 border-b border-gray-300">
                <h1 class="text-lg font-semibold text-gray-700 flex"> <span @click="router.back()"
                    ...
                    <ListBulletIcon class="h-7 w-7 hidden md:flex mr-2" /> Liste de series
                </h1>
                <div class="w-[15%] flex items-center justify-between">
                    <span class="h-8 w-8 md:mr-2 p-1 m-1 rounded bg-gray-300 grid place-items-center">{{sets.length == 0 | loading ? "" : sets.length }}</span>
                    <router-link to="/admin/sets/create"><button
            ...
            <div class="w-full text-gray-700 mb-3 font-semibold flex justify-between border-b border-gray-300 p-2">
                ...
                <span class="w-3/12 text-ellipsis overflow-hidden">Modèle</span>
                <span class="md:w-1/12 w-2/12">Modif.</span>
                <span class="w-1/12 text-center">---</span>
            </div>
            <div v-if="!loading && sets.length > 0" v-for="s, i of sets" class="flex flex-col">
                <div :key="s.mar_id"
                    ...
                    <span class="md:w-3/12 w-5/12 font-semibold">{{ s.set_name }}</span>
                    <span class="w-3/12 text-ellipsis overflow-hidden">{{ s.mod_name }}</span>
                    <span class="md:w-1/12 w-2/12">{{ formatDateToAgo(s.last_update) }}</span>
                    <span class="w-1/12 flex justify-center"><router-link
                            :to="'/admin/sets/' + s.set_id + '/' + prettyString(s.set_name) + '/c/' + s.cat_id + '/mod/' + s.mod_id + '/mar/' + s.mar_id + '/update'"><span
            ...
            <div v-else class="flex flex-col items-center justify-center h-60">
                <NoData name="aucune serie" link="/admin/sets/new" />
            </div>
        </div>
    </div>
</template>
<script setup>
import { ListBulletIcon, ArrowLeftIcon, PencilIcon } from "@heroicons/vue/24/solid"
...
async function getSets() {
    loading.value = true
    const result = await Set.getAll()
    if (result.type === "success") {
        sets.value = !Array.isArray(result.data) ? [result.data] : result.data
    }
    loading.value = false
}
onMounted(getSets)
