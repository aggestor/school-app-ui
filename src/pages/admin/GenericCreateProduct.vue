                    ...
                    <div class="w-full h-full">
                        <div class="w-full h-full flex space-y-3 flex-col justify-between">
                            <div  class="w-11/12 mx-auto ">
                                <span class=" my-3 w-72 text-sm">Taper le nom du <b>shop</b>  puis choisissez parmis les suggestions pour poursuivre la creation de votre produit</span> 
                                <div class="relative max-h-96 mt-1 border rounded p-1 w-full">
                                    <input @input="handleSearchShops" :value="searchBoxValueShop" class="h-8 w-full outline-none" placeholder="Rechercher un shop" />
                                    <div data-aos-duration="300" v-if="showShopSuggestionsBox" class=" w-full shadow z-50 absolute top-10 rounded p-2 h-60 bg-white border left-0 mt-1">
                                    <SearchSkeltonAnimation v-if="loading"/>
                                        <div class="w-full" v-else-if="!loading && Array.isArray(shops)">
                                            <div @click="() => chooseSetShop(s)" class="w-full hover:bg-gray-100 cursor-pointer rounded p-1 flex justify-between" v-for="s of shops" :key="s.id">
                                                <span>{{s.name}}</span>  <span class="text-xs w-4/12 overflow-hidden  text-ellipsis">{{s.categories.replace("$", ",")}} | <span class="text-yellow-600">{{s.level}}</span></span>
                                            </div>
                                        </div>
                                        ...
                                        <div v-else class="w-full h-full grid text-gray-600 place-items-center">
                                            <div class="w-fit flex flex-col items-center h-fit">
                                                ...
                                                <small class="w-9/12 mx-auto text-center">Aucun resultat pour ce nom de
                                                    série. Verifier l'orthographe ou <router-link
                                                        class="text-blue-600 font-semibold" to="/admin/sets/new">créer
                                                        d'abord cette série</router-link> .</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="relative max-h-96 mt-1 border rounded p-1 w-full">
                                    ...
                                    <div data-aos-duration="300" v-if="showTypeSuggestionsBox"
                                        ...
                                        <div v-else class="w-full h-full grid text-gray-600 place-items-center">
                                            <div class="w-fit flex flex-col items-center h-fit">
                                                <FaceFrownIcon class="h-16 w-16 mb-2" />
                                                <small class="w-9/12 mx-auto text-center">Aucun resultat pour ce type. Verifier l'orthographe ou <router-link
                                                        class="text-blue-600 font-semibold" to="/admin/types/new">créer
                                                        d'abord ce type</router-link> .</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="w-11/12 pt-10 relative rounded  mx-auto">
                                <div class=" flex justify-around flex-col items-center w-full">
                                    <span
                                        class="absolute flex items-center top-1 left-0 rounded  text-gray-700 font-semibold ">
                                    ...
                                    <div class="flex w-full max-h-[400px] overflow-auto rounded p-2 flex-col">
                                        ...
                                        <div class="w-full flex justify-between  h-32">
                                            ...
                                            <div v-else class="w-full flex justify-evenly flex-wrap  md:justify-between">
                                                <div v-for="i of imageURL" class="w-20 md:w-24 h-20 md:h-24 m-2 rounded overflow-auto">
                                            ...
                                            <!-- <div v-if="imagesFiles.length < 6" @click="handleAddImage"
                                                class="h-20 md:h-24 w-20 md:w-24 cursor-pointer grid place-items-center border rounded">
                                                <span class=""><svg class="h-10 fill-gray-600"
                                                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                                        <path
...
export default {
    ...
    methods: {
        ...
        async onClickCreateProduct() {
            ...
            const data = { ...this.currentLinkType,prices,quantities, ...this.currentLink, name:this.name, description:this.description,files:this.imagesFiles,specs:JSON.stringify(this.chosenSpecs), isBrandNew:this.isBrandNew,keywords: this.keywords.concat(this.keywordsDefault).join(","),...mark_}
        ...
        async handleSearchLinks(e) {
            ...
            if (value && value.length >= 2) {
                ...
                this.loading = true
                const result = await _Set.searchLinks(this.$route.params.id || this.currentShop.id,value)
                if (result.type === 'success') {
