    ...
    <div v-else data-aos="slide-up" data-aos-duration="500" class="md:w-10/12 w-full bg-white __shad flex justify-between items-center border h-[92%] rounded-lg">
        <div class="md:w-7/12 flex h-[80%] md:p-6 p-4  justify-center flex-col" >
            <div class=" flex items-center">
                <GoBackAdminButton/> <span class="ml-1 mr-1">|</span> <h1 class="text-gray-700">Créer une branche pour <b>{{ shop.name }}</b> </h1>
            </div>
            <form class="h-full w-full flex flex-col">
                <div class="md:w-5/12 my-3 w-full flex md:hidden flex-col items-center justify-center h-full">
                    <div class="h-60 md:w-60 w-full relative flex justify-center items-center rounded border">
                        <div v-if="!image" class="flex justify-center flex-col items-center">
                            ...
                            <span>Ajouter un logo pour votre branche</span>
            ...
            <div class="w-full items-center mt-2 flex  justify-between">
                <ButtonLink text="Voir les branches de ce shop" :path="'/admin/shops/'+shop.id+'/branches'" variant="default"/>
...
export default {
    ...
    methods: {
        ...
        async onPressRegister() {
            const result = await Branch.create({name:this.name,town:this.locationId,location:this.town,image:this.fileImage, owner:this.shop.owner_id, shop_id:this.shop.id, contact:this.contact,whatsApp:this.whatsApp,address:this.address})
