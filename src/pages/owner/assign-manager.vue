<template>
    <div class="w-full md:h-[90%] flex justify-center items-center">
        <div v-if="!promptSuccessBox"  class="lg:w-6/12 md:w-8/12 border w-11/12 md:mb-5 mb-3 mt-10  bg-white __shad  shadow-gray-200 flex h-auto rounded p-8  justify-center flex-col" >
            <PagesNavigationHeader title="Assigner un gerant a une branche"/>
            <div class="flex w-full my-2 justify-between items-center">
                <span >Nom de la branche</span>
                <span class="font-semibold">{{prettyString(this.$route.params.name,true)}}</span>
            </div>
            <p class="text-gray-600 mb-3 text-sm">Choisissez le manager a assossier a cette branche</p>
            <form class="h-full w-full flex flex-col">
                <div class="relative max-h-96 mt-1 border rounded p-1 w-full">
                                    <input @input="handleSearchBox" :value="searchBoxValue" class="h-8 w-full outline-none" :placeholder="`Taper le nom du gerant à associer à ${prettyString( this.$route.params.name,true)}`" />
        ...
        <div v-else class="md:w-7/12 border w-[95%] md:mb-5 mb-3 mt-10 md:h-80  bg-white __shad  shadow-gray-200 flex rounded justify-center flex-col">
            <SuccessComponent :path="'/owner/branches/'+this.$route.params.id" title="Assignement effectué !" message="L'utilisateur a été assigné avec succès. Il a maintenant un role de gerant dans votre branche partout. Vous pouvez modifier ou annuler cette action apres.."/>
        </div>
    </div>
</template>
<script>
    ...
    methods: {
        ...
        async handleSearchBox(e) {
            if(e.target.value.length > 2){
                ...
                this.loading = true
                const result = await User._search(e.target.value)
        ...
        async onPressAssign() {
            const result = await User.assignManagerToBranch_({shop:this.$route.params.shop_id, branch:this.$route.params.id, user: this.user.id})
