<template>
    ...
    <div class="w-11/12 mx-auto  flex justify-center items-center h-auto">
        <div v-if="promptSuccessBox" data-aos="slide-up" data-aos-duration="500"
            class="md:w-8/12 w-full overflow-hidden __shad flex md:flex-row flex-col justify-between items-center h-96 md:h-[350px] rounded-lg border">
            <div
            ...
            <div class="md:w-6/12 w-full h-[50%] flex items-center flex-col justify-center md:h-full">
                ...
                <span
                    class="bg-green-500 shadow-green-300 shadow-lg px-2 py-1.5 mt-5 hover:bg-green-600 rounded text-white text-lg"><router-link
                        :to="'/admin/products/' + shop.id + '/branches'">Continuer</router-link></span>
            </div>
        </div>
                                ...
                                <div class=" flex justify-around flex-col items-center w-full h-full">
                                    ...
                                    <div class="w-full flex flex-wrap rounded border p-1 h-32">
                                        <span v-for="k of keywordsDefault"
                                            class="bg-gray-200 mr-1 h-fit rounded-full px-2 flex justify-center w-fit">{{ k }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="w-11/12 pt-10 relative rounded  mx-auto">
                            ...
                            <div class="w-11/12 pt-10 relative rounded  mx-auto">
                                    ...
                                    <div class="flex w-full max-h-[400px] overflow-auto rounded p-2 flex-col">
                                        ...
                                        <div class="w-full h-32">
                                            <div v-if="!imageURL[0]" @click="handleChooseImage"
                                                class="h-28 w-28 cursor-pointer grid place-items-center border rounded">
                                            ...
                                            <div v-else class="w-full flex justify-between">
                                                <div v-for="i of imageURL" class="w-24 h-24 rounded overflow-hidden">
                                ...
                                <div class="w-full mt-2 rounded flex justify-between">
                                    ...
                                    <span @click="onClickCreateProduct"
                                        class="h-10 w-fit  cursor-pointer px-1 font-semibold text-white rounded bg-blue-600 flex items-center justify-between">Finaliser
...
export default {
    ...
    methods: {
        ...
        async onClickCreateProduct() {
            ...
            const result = await Product.create_(data)
        ...
        async handleSearchLinks(e) {
            ...
            if (value && value.length >= 2) {
                ...
                const result = await _Set.searchLinks_(this.shop.id,value)
                if (result.type === 'success') {
        ...
        async handleSearchLinksTypes(e) {
            ...
            if (value && value.length >= 2) {
                ...
                this.loading = true
                const result = await Type.searchLinks_(this.currentLink.category_id,value)
                if (result.type === 'success') {
                    if (Array.isArray(result.data) && result.data.length > 0) {
        ...
        async getSpecs() {
            const result = await Spec.getByType(this.currentLinkType.type_id)
            if (result.type === "success") {
